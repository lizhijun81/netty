...
/**
 * ChannelPipeline 是 ChannelHandler 的List，用于处理 Channel 的 inbound events 和 outbound operations。
 *
 * ChannelPipeline 的 实现是 优先级的 拦截器
 *
 * 每个 channel 都有自己的pipeline，pipeline 是在在channel创建时自动创建
 *
 * event 在 pipeline 中的发生顺序
 *  I/O 事件被处理 ChannelInboundHandler 或者 ChannelOutboundHandler。并且 被转发给最近的Handler
 *  I/O 事件在传播方法 定义在ChannelHandlerContext
 *
 *
 ...
 * diagram.  An inbound event 子底向上执行inbound handlers。
 *
 * An inbound handler usually handles the inbound data generated by the I/O thread on the bottom of the
 * diagram.  An inbound handler 通常处理 I/O thread 产生的 inbound data。
 *
 *
 ...
 *
 * inbound data 通常被读取从远程 通过 真实的 输入操作，比如：read().
 * 如果inbound event 发生 超过最上边的inbound handler，该事件将被丢弃
 *
 * <p>
 ...
 *
 * Inbound event 的 Handler 的传播：
 *      Registered 事件 通过 执行 fireChannelRegistered()
 *      Read 事件 通过执行 fireChannelRead() 。。。
 * Outbound event 的 Handler 的传播：
 *      bind 事件 通过 执行 bind()
 *      write 事件 通过执行 write() 。。。
 *
 ...
 *
 * 不同线程的 pipeline； 执行 MyBusinessLogicHandler 线程 和 I/O操作的线程不同；
 * MyBusinessLogicHandler 可以用来处理长时间执行的 Task
